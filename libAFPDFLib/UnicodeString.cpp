...
{
	return Mid(m_Length - dwCount - 1, dwCount);
}
...
{
	m_Length = m_MaximumLength = (USHORT)dwSize;
	if(m_Buffer)
		free(m_Buffer);
	m_Buffer = (LPWSTR)malloc((dwSize + 1) * sizeof(WCHAR));
}

LPWSTR CUnicodeString::GetBuffer()
{
	return m_Buffer;
}
...
void CUnicodeString::FormatMessage(UINT nFormatID, ...)
{
	...
	LPWSTR lpszTemp;
	if (::FormatMessageW(FORMAT_MESSAGE_FROM_STRING|FORMAT_MESSAGE_ALLOCATE_BUFFER,(const char*) strFormat, 0, 0, (LPWSTR)&lpszTemp, 0, &argList) == 0 || lpszTemp == NULL)
		return;
...
void CUnicodeString::FormatMessage(LPCSTR lpszFormat, ...)
{
	...
	m_Length = iLen;
	m_Buffer = (LPWSTR)malloc((iLen + 1) * sizeof(WCHAR));
	MultiByteToWideChar(CP_ACP, 0, lpszTemp, m_Length, m_Buffer, m_Length);
	LocalFree(lpszTemp);
	va_end(argList);
}
...
CUnicodeString::operator const LPCSTR ()
{
	...
	WideCharToMultiByte(CP_ACP, 0, m_Buffer, m_Length, m_TempBuff, m_Length, NULL, NULL);
...
{
	m_usBuffer.Length = m_usBuffer.MaximumLength = m_Length * sizeof(WCHAR);
